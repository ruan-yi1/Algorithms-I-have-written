...
int main(void) 
{
        uint8 currentStatus = 0; // 记录状态
        uint64 expandNumber = 0; // 记录指令数
        int16 inputChar = 0; // 记录用户输入

        print("请输入一个以形如A{3B}的字符串, ctrl+Z/D结束程序: \n");
        print("User> ");
        fflush(stdout);

        while ( (inputChar = (int16)getchar() ) != EOF) {
                ...
                switch (currentStatus) {
                        ...
                        default :
                                if (isdigit(inputChar)) {
                                        expandNumber = expandNumber * 10 + (uint64)(inputChar - '0');
                                }
                                else if (inputChar == '}') {
                                        expandNumber = 0;
                                        currentStatus = 0;
                                        ...
                                        if (expandNumber > 0) {
                                                for (uint64 k = 0; k < expandNumber; k++) {
                                                        putchar(inputChar);
                                                }
                                                expandNumber = 0;
                                        }
